...
- 学习笔记中的原始表述、推导过程、未完成项和疑问属于用户的学习轨迹。除非用户明确要求重写，否则优先保留原有结构和措辞。
- 默认采用评审和建议模式：在用户原文后追加“澄清”“补充”“建议”或评论，不直接用完整答案替换用户自己的理解。
- 用户尚未填写的 TODO 或问题，不要直接代写答案；应提供可执行的学习途径，例如源码入口、文档章节、观察问题和最小实验。
- 必须清楚区分用户原有内容与 Codex 新增内容，避免整理后无法判断哪些结论是用户自己形成的。
- 对文档做大规模重构、改写章节结构或改变表达形式前，先提出调整建议并取得用户明确同意。
- 只有当用户明确要求“直接整理成完整文档”“代写”或“重构”时，才可以主动改写；即使如此，也应尽量保留有价值的原始思考。
